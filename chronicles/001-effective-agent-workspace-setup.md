...
- Keep it tight, but for the right reason. The usual argument is that every line costs you adherence elsewhere. A factorial study varying file size, instruction position, file architecture, and contradictions between adjacent files found none of them detectably changed whether the agent followed its instructions — and for size, positive evidence of no effect rather than merely a failure to find one. Read the scope honestly before treating that as settled: it measured compliance with one trivial annotation, not adherence in general, and one narrow measure carries only so far. Still, it is the only direct test of the claim, and it does not support it. What survives is cost: every line is paid for on every turn, forever, and the benefit it buys is unmeasured. Targeting a few hundred lines or fewer remains good discipline on those grounds alone, and vendor guidance sits at the low end of that range.
